...
# Correção robusta para ambientes onde o Streamlit Cloud não instalou dependências do requirements.txt.
try:
...
menu = st.sidebar.radio(
    "Navegação",
    ["▦ Visão Geral","▤ Projetos","🔎 Detalhe do Projeto","◈ Análise Financeira","◷ Atrasos e Prazos","⌖ Mapa de Localizações","▥ Planta da Unidade","⚠ Alertas","◎ Previsão (EVM)","∿ Curva S","ⓘ Notas Explicativas","⚙ Configurações"],
...
<div class="header">
  <div style="display:flex;justify-content:space-between;gap:16px;align-items:start;">
    <div>
      <div class="header-title">{menu.replace('▦ ','').replace('▤ ','').replace('🔎 ','').replace('◈ ','').replace('◷ ','').replace('⌖ ','').replace('▥ ','').replace('⚠ ','').replace('◎ ','').replace('∿ ','').replace('ⓘ ','').replace('⚙ ','')}</div>
...
if menu == "▦ Visão Geral":
    ...
    st.download_button("Baixar análise consolidada em Excel", export_excel(df), file_name="capexvision_v21_4_analise.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
