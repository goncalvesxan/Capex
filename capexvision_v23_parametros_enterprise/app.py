    ...
    if bg:
        ...
        <div class="floor">
            ...
            <div class="floor-empty-label">
                Nenhuma imagem vinculada à empresa selecionada<br>
                Cadastre a imagem em Empresas & Plantas.
            </div>
        """
...
def get_companies_from_data(df_source):
    if "empresa" in df_source.columns:
        return sorted([str(x) for x in df_source["empresa"].dropna().astype(str).unique().tolist()])
    return []
...
def get_company_floorplan(selected_company, df_filtered):
    ...
    if selected_company and selected_company != "Todas":
        return selected_company, images.get(str(selected_company))

    if "empresa" in df_filtered.columns:
        for emp in df_filtered["empresa"].dropna().astype(str).unique().tolist():
            if emp in images:
                return emp, images.get(emp)

    if images:
        first = list(images.keys())[0]
        return first, images[first]
...
if menu == "▦ Visão Geral":
    ...
    st.markdown('<div class="story"><b>Central de Governança & Parâmetros:</b> ajuste os coeficientes que afetam risco, health score, maturidade, CAPEX/OPEX, What-if, ROI, ESG e qualidade de dados.</div>', unsafe_allow_html=True)

    tabs = st.tabs(["Risco & Alertas","Health & Maturidade","CAPEX x OPEX","What-if & Forecast","ROI / ESG / Dados","Upload Tabela","Auditoria"])

    with tabs[0]:
        c1,c2,c3 = st.columns(3)
        with c1:
            v = st.number_input("CPI amarelo", 0.0, 2.0, float(get_param("risk_cpi_yellow",.9)), .01)
            audit_param_change("risk_cpi_yellow", get_param("risk_cpi_yellow"), v); st.session_state.engine_params["risk_cpi_yellow"] = v
            v = st.number_input("CPI vermelho", 0.0, 2.0, float(get_param("risk_cpi_red",.8)), .01)
            audit_param_change("risk_cpi_red", get_param("risk_cpi_red"), v); st.session_state.engine_params["risk_cpi_red"] = v
        with c2:
            v = st.number_input("SPI amarelo", 0.0, 2.0, float(get_param("risk_spi_yellow",.9)), .01)
            audit_param_change("risk_spi_yellow", get_param("risk_spi_yellow"), v); st.session_state.engine_params["risk_spi_yellow"] = v
            v = st.number_input("SPI vermelho", 0.0, 2.0, float(get_param("risk_spi_red",.8)), .01)
            audit_param_change("risk_spi_red", get_param("risk_spi_red"), v); st.session_state.engine_params["risk_spi_red"] = v
        with c3:
            v = st.number_input("VAC amarelo (%)", -1.0, 1.0, float(get_param("risk_vac_yellow_pct",-.05)), .01)
            audit_param_change("risk_vac_yellow_pct", get_param("risk_vac_yellow_pct"), v); st.session_state.engine_params["risk_vac_yellow_pct"] = v
            v = st.number_input("VAC vermelho (%)", -1.0, 1.0, float(get_param("risk_vac_red_pct",-.10)), .01)
            audit_param_change("risk_vac_red_pct", get_param("risk_vac_red_pct"), v); st.session_state.engine_params["risk_vac_red_pct"] = v

    with tabs[1]:
        st.markdown("##### Pesos do Health Score")
        cols = st.columns(5)
        keys = ["health_weight_cpi","health_weight_spi","health_weight_vac","health_weight_forecast","health_weight_data_quality"]
        labels = ["CPI","SPI","VAC","Forecast","Data Quality"]
        for col,k,l in zip(cols,keys,labels):
            with col:
                v = st.number_input(l, 0, 100, int(get_param(k,20)), 1, key=f"input_{k}")
                audit_param_change(k, get_param(k), v); st.session_state.engine_params[k] = v

        st.markdown("##### Pesos do Maturity Score")
        cols = st.columns(5)
        keys = ["maturity_weight_budget","maturity_weight_schedule","maturity_weight_physical_progress","maturity_weight_cost_performance","maturity_weight_forecast"]
        labels = ["Budget","Cronograma","Avanço Físico","Custo","Forecast"]
        for col,k,l in zip(cols,keys,labels):
            with col:
                v = st.number_input(l, 0, 100, int(get_param(k,20)), 1, key=f"input_{k}")
                audit_param_change(k, get_param(k), v); st.session_state.engine_params[k] = v
    ...
    with tabs[2]:
        ...
        with c1:
            ...
            audit_param_change("capex_min_value", get_param("capex_min_value"), v); st.session_state.engine_params["capex_min_value"] = v
        with c2:
            v = st.text_area("Palavras-chave OPEX", str(get_param("capex_opex_keywords","")), height=100)
            audit_param_change("capex_opex_keywords", get_param("capex_opex_keywords"), v); st.session_state.engine_params["capex_opex_keywords"] = v
        with c3:
            v = st.number_input("Payback limite OPEX (meses)", 0, 120, int(get_param("capex_payback_limit_months",12)), 1)
            audit_param_change("capex_payback_limit_months", get_param("capex_payback_limit_months"), v); st.session_state.engine_params["capex_payback_limit_months"] = v

    with tabs[3]:
        c1,c2,c3,c4 = st.columns(4)
        with c1:
            v = st.number_input("Corte orçamento padrão (%)", 0, 80, int(get_param("whatif_default_budget_cut_pct",10)), 1)
            audit_param_change("whatif_default_budget_cut_pct", get_param("whatif_default_budget_cut_pct"), v); st.session_state.engine_params["whatif_default_budget_cut_pct"] = v
        with c2:
            v = st.number_input("Atraso padrão (meses)", 0, 36, int(get_param("whatif_default_delay_months",3)), 1)
            audit_param_change("whatif_default_delay_months", get_param("whatif_default_delay_months"), v); st.session_state.engine_params["whatif_default_delay_months"] = v
        with c3:
            v = st.number_input("Aumento custo/câmbio padrão (%)", -50, 100, int(get_param("whatif_default_fx_cost_pct",8)), 1)
            audit_param_change("whatif_default_fx_cost_pct", get_param("whatif_default_fx_cost_pct"), v); st.session_state.engine_params["whatif_default_fx_cost_pct"] = v
        with c4:
            v = st.selectbox("Modelo de forecast", ["Híbrido","Linear","Curva S","Regressão","IA"], index=["Híbrido","Linear","Curva S","Regressão","IA"].index(str(get_param("forecast_model","Híbrido"))))
            audit_param_change("forecast_model", get_param("forecast_model"), v); st.session_state.engine_params["forecast_model"] = v

    with tabs[4]:
        c1,c2,c3,c4 = st.columns(4)
        with c1:
            v = st.number_input("Peso ROI Health", 0, 100, int(get_param("roi_weight_health",45)), 1)
            audit_param_change("roi_weight_health", get_param("roi_weight_health"), v); st.session_state.engine_params["roi_weight_health"] = v
        with c2:
            v = st.number_input("Peso ROI Maturidade", 0, 100, int(get_param("roi_weight_maturity",25)), 1)
            audit_param_change("roi_weight_maturity", get_param("roi_weight_maturity"), v); st.session_state.engine_params["roi_weight_maturity"] = v
        with c3:
            v = st.number_input("Peso ROI Risco", 0, 100, int(get_param("roi_weight_risk",30)), 1)
            audit_param_change("roi_weight_risk", get_param("roi_weight_risk"), v); st.session_state.engine_params["roi_weight_risk"] = v
        with c4:
            v = st.number_input("Bônus palavra-chave ESG", 0, 100, int(get_param("esg_keyword_bonus",30)), 1)
            audit_param_change("esg_keyword_bonus", get_param("esg_keyword_bonus"), v); st.session_state.engine_params["esg_keyword_bonus"] = v

        v = st.text_area("Campos obrigatórios Data Quality", str(get_param("data_quality_required_fields","")), height=90)
        audit_param_change("data_quality_required_fields", get_param("data_quality_required_fields"), v); st.session_state.engine_params["data_quality_required_fields"] = v

    with tabs[5]:
        st.markdown("Faça upload de um Excel/CSV com colunas `parametro` e `valor` para atualizar parâmetros em massa.")
        file_params = st.file_uploader("Upload tabela de parâmetros", type=["xlsx","csv"], key="param_upload_table")
    ...
    with tabs[6]:
        ...
        st.download_button("Baixar parâmetros atuais", pd.DataFrame([st.session_state.engine_params]).to_csv(index=False).encode("utf-8"), "parametros_capexvision.csv", "text/csv")

elif menu == "🏢 Empresas & Plantas":
    st.markdown('<div class="story"><b>Empresas & Plantas:</b> o sistema identifica as empresas do arquivo carregado e permite vincular uma imagem de planta a cada empresa. Ao mudar o filtro Empresa, a planta muda automaticamente.</div>', unsafe_allow_html=True)

    empresas_detectadas = get_companies_from_data(s if "s" in globals() else df)
    if not empresas_detectadas:
        st.warning("Nenhuma empresa identificada nos dados carregados.")
    else:
        cols = st.columns(2)
        with cols[0]:
            selected_company_upload = st.selectbox("Empresa", empresas_detectadas, key="company_floorplan_select")
            uploaded_company_img = st.file_uploader("Upload planta da empresa", type=["png","jpg","jpeg"], key=f"company_floorplan_upload_{selected_company_upload}")
            if uploaded_company_img is not None:
                st.session_state.company_floorplan_images[selected_company_upload] = image_to_data_url(uploaded_company_img)
                st.session_state.company_floorplan_names[selected_company_upload] = uploaded_company_img.name
                st.success(f"Imagem vinculada à empresa {selected_company_upload}.")
            if st.button("Remover imagem desta empresa"):
                st.session_state.company_floorplan_images.pop(selected_company_upload, None)
                st.session_state.company_floorplan_names.pop(selected_company_upload, None)
                st.success("Imagem removida.")

        with cols[1]:
            st.markdown('<div class="ptitle">EMPRESAS DETECTADAS</div>', unsafe_allow_html=True)
        ...
        with st.container(border=True):
            st.markdown('<div class="ptitle">PRÉ-VISUALIZAÇÃO DA PLANTA</div>', unsafe_allow_html=True)
            preview_df = df[df["empresa"].astype(str)==selected_company_upload] if "empresa" in df.columns else df
        ...
        if st.session_state.company_floorplan_names:
            st.dataframe(pd.DataFrame([{"Empresa":k,"Arquivo":v} for k,v in st.session_state.company_floorplan_names.items()]), use_container_width=True, hide_index=True)
    ...
    st.download_button("Baixar análise consolidada em Excel", export_excel(df), file_name="capexvision_v23_analise.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
