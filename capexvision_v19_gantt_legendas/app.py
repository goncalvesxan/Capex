...
if menu == "▦ Visão Geral":
    ...
    st.write("Registros no log:", len(st.session_state.load_log))
        st.write("Imagem da planta carregada:", st.session_state.floorplan_name if st.session_state.floorplan_name else "Nenhuma")
    st.dataframe(pd.DataFrame(st.session_state.load_log), use_container_width=True)
    st.download_button("Baixar análise consolidada em Excel", export_excel(df), file_name="capexvision_v18_analise.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
